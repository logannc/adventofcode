...
fn is_valid_part_one(candidate: u32) -> bool {
    let digits: Vec<u32> = candidate.to_string().chars().flat_map(|c| c.to_digit(10)).collect();
    // has any two same digits adjacent
    let (has_adjacent, _) = digits.iter().skip(1).fold((false, digits[0]),
    |(found, prev), curr|{
        (found || prev == *curr, *curr)
    });
    // left-to-right monotonic non-decreasing
    let (monotonic, _) = digits.iter().skip(1).fold((true, digits[0]),
    |(nondecreasing, prev), curr|{
...
fn is_valid_part_two(candidate: u32) -> bool {
    let digits: Vec<u32> = candidate.to_string().chars().flat_map(|c| c.to_digit(10)).collect();
    // has any two same digits adjacent
    let (found, last_run_length, _) = digits.iter().skip(1).fold((false, 1, digits[0]),
    |(found, run_length, prev), curr|{
